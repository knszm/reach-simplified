...
import numpy as np
import canonical, hamiltonians, dla, spectral
...
#    if check_dla:
#        out = hamiltonians.select_random_full_dla(ham_key, H_full, K, timeout)
#        if out is None:                       # timed out
#            return dict(run=run, timed_out=1)
#        H_sub, idx = out
#        dla_full = 1                          # found => full by construction
    else:
   ...
   #     if best_ov>threshold:
   #         break
   # return dict(run=run, timed_out=0, dla_full=dla_full,
   #             overlap=best_ov, n_rejected=n_rejected,
   #             indices=" ".join(map(str, idx)),
   #             lam_opt=" ".join(map(str, np.asarray(best_lam))))
#    return dict(run=run, timed_out=0, dla_full=dla_full,
#                overlap=best_ov,
...
#HEADER = ["run","dim","K","seed","check_dla","dla_full","timed_out",
#          "overlap","indices","lam_opt","n_starts","timestamp"]
...
def main():
    ...
    os.makedirs("spectral_canonical_results", exist_ok=True)
    path = os.path.join("spectral_canonical_results",
